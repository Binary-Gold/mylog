...
uint8_t* MMapAux::Data() { return Handle_() + sizeof(MMapHeader); }
...
uint8_t* MMapAux::Handle_() { return static_cast<uint8_t*>(imp_->mmap_->Data()); }
...
void MMapAux::Trymap_(uint64_t valid_new_size) { imp_->mmap_ = std::make_unique<fs::MMap>(*imp_->fd_, valid_new_size); }
}  // namespace mmap
