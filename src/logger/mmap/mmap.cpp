...
struct MMapAux::Imp {
    ...
    std::unique_ptr<logger::fs::Fd> fd_;
    std::unique_ptr<logger::fs::MMap> mmap_;  // Trymap_ 时延迟创建
};

MMapAux::MMapAux(const fpath& file_path) : imp_(std::make_unique<Imp>()) {
    imp_->fd_ = std::make_unique<logger::fs::Fd>(file_path);
...
uint64_t MMapAux::Size() const { return Header_()->data_size_; }
void MMapAux::Resize(uint64_t new_size) {
    Reserve_(sizeof(MMapHeader) + new_size);
    Header_()->data_size_ = new_size;
}

uint8_t* MMapAux::Data() const { return Handle_() + sizeof(MMapHeader); }

void MMapAux::Clear() { Header_()->data_size_ = 0; }
...
double MMapAux::Ratio() const {
    if (imp_->capacity_ - sizeof(MMapHeader) == 0) throw std::runtime_error("Ratio 0 error");
    return static_cast<double>(Size()) / (imp_->capacity_ - sizeof(MMapHeader));
}

void MMapAux::Init_() {
    MMapHeader* header = reinterpret_cast<MMapHeader*>(Handle_());
...
uint64_t MMapAux::GetValidCapacity_(uint64_t size) {
    uint64_t page_size = logger::fs::GetPageSize();
    return (size + page_size - 1) / page_size * page_size;
}

void MMapAux::Reserve_(uint64_t new_size) {
    if (new_size <= imp_->capacity_) return;
...
uint8_t* MMapAux::Handle_() const { return static_cast<uint8_t*>(imp_->mmap_->Data()); }

MMapHeader* MMapAux::Header_() const {
    MMapHeader* header = reinterpret_cast<MMapHeader*>(imp_->mmap_->Data());
    if (header->magic_ != MMapHeader::kMagic) throw std::runtime_error("Header_() failed");
    return header;
}
void MMapAux::Unmap_() { imp_->mmap_.reset(); }
void MMapAux::Trymap_(uint64_t valid_new_size) { imp_->mmap_ = std::make_unique<logger::fs::MMap>(*imp_->fd_, valid_new_size); }
}  // namespace logger::mmap
